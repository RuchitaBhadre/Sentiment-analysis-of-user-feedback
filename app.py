...
def feedback_post():
    ...
    #feedback received is in string and input to text analytics has to be a document so convert it to one as follows:
    documents=[
    ...
    print("Let's visualize the sentiment of the feedback")
    for idx, doc in enumerate(senti):
        overall_sentiment=doc.sentiment
        print(f"Document Text: {documents[idx]}")
        print(f"Overall sentiment: {doc.sentiment}")
    
    #Preparing input to the LLM model
